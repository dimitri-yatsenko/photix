...
class ETracer:
    """
    A ray tracer for a specific emitter shapes.
    Measures flux at points given points in space
    """

    def __init__(self, emitter_shape, emitter_size, emitter_profile, detector_positions, anisotropy,
                 absorption_length, scatter_length, beam_compression, beam_xy_aspect, y_steer, max_hop):
        ...
        self.n = 4096  # photon packets at a time
        self.positions = np.empty((self.n, 3))  # (um)   x, y, z
        self.directions = np.empty((self.n, 3))  # (um)   x, y, z
        self.max_hop = max_hop  # (um)
        self.cell_radius = 5.0  # (um) for measuring flux
        self.detector_positions = detector_positions
        self.detector_tree = spatial.cKDTree(self.detector_positions)

        # simulation parameters
        self.emitter_profile = emitter_profile  # e.g. lambertian, parallel
        ...
        self.flux = np.zeros(detector_positions.shape[0], dtype='float32')
        self.photon_count = 0  # total emitted photons

    def emit(self, photons_to_replace):
        """
        Emits photons from the emitter
        :param photons_to_replace: boolean indices of the photons to generate
        """
        # generate positions
        ...
        # generate directions
        if self.emitter_profile == "spherical":
            directions = np.random.randn(m, 3)
            directions /= np.sqrt((directions ** 2).sum(axis=1, keepdims=True))
        elif self.emitter_profile == "lambertian":  # oriented along z-axis
    ...
    def hop(self, iteration):
        """
        execute photon hops between events, updating the irradition field
        """
        # irradiate
        lengths = np.random.exponential(1. / self.mu, self.n)
        keep = lengths > self.max_hop  # keeps going - no interaction
        lengths[keep] = self.max_hop
        new_positions = self.positions + lengths[:, None] * self.directions

        # count detections
        search_radius = np.sqrt((self.max_hop / 2) ** 2 + self.cell_radius ** 2)
        for start, end, detectors in zip(
                self.positions, new_positions,
                self.detector_tree.query_ball_point(
                    (self.positions + new_positions) / 2, search_radius)):
            if detectors:
                u = self.detector_positions[detectors] - start
                v = end - start
                v_fraction = (u @ v) / (v ** 2).sum()
                distance = np.sqrt(((u - np.outer(v_fraction, v)) ** 2).sum(axis=1))
                within = (v_fraction > 0) * (v_fraction < 1) * (distance < self.cell_radius)
                self.flux[np.array(detectors)[within]] += 1

        self.positions = new_positions

        # replace absorbed or escaped
        max_distance = 250
        strayed = iteration % 20 == 0 and (
                0 == self.detector_tree.query_ball_point(new_positions, max_distance, return_length=True))
        absorbed = ~keep * (np.random.rand(self.n) > self.absorb_ratio)  # absorbed in tissue
        self.emit(absorbed + strayed)

        # scatter: gcos follows the Henyey-Greenstein distribution of cosines
        scattered = ~(keep + absorbed + strayed)
        m = scattered.sum()
        g = self.anisotropy
    ...
    def run(self, nhops, display_progress=True):
        self.emit(np.ones(self.n, dtype=np.bool))
        for i in (tqdm.tqdm(range(nhops), miniters=20) if display_progress else range(nhops)):
            self.hop(i)
        self.flux = self.flux / (np.pi * self.cell_radius ** 2) / self.photon_count


class SpaceTracer:
    """
    VolumeTracer computes emission flux or detection probability in 3D Volume
    from an emitter or detector placed in the center
    """
    ...
    def detector_sensitivity(self):
        ...
        if self.detector_type == "narrowed4":
    ...
    def accumulate(self, start_points, end_points, lengths):
        ...
        for i in range(samples):
            ...
            positions = np.round(
                dims / 2 + (start_points * fractions + end_points * (1 - fractions)) / self.pitch).astype(np.int32)
    ...
    def hop(self):
        ...
        absorbed = np.logical_or(np.random.rand(self.n) < self.absorb_ratio,
